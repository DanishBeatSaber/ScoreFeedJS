...
request.open("POST", "Webhook"); //Set Webhook
...
var params = { //Create Params
	tts: false, //TTS
	username: "Dane Saber - Top scores", //Username
...
TAsock.onmessage = async function(event) { // event.data is the message
	...
	jsonObj = JSON.parse(event.data); // parse the message as JSON
		...
		//console.log(""+jsonObj.commandData.score.leaderboardPlayerInfo.name+" ("+jsonObj.commandData.score.leaderboardPlayerInfo.id+") from "+jsonObj.commandData.score.leaderboardPlayerInfo.country+" just set a score on "+jsonObj.commandData.leaderboard.songHash+" ("+jsonObj.commandData.leaderboard.songName+" - "+jsonObj.commandData.leaderboard.songAuthorName+" by "+jsonObj.commandData.leaderboard.levelAuthorName+")"); //Log everything
		//const bannedIDs = [If you really need to ban someone i guess?]
		
		if (jsonObj.commandName === "score") /*Check for command*/ {
			//if (jsonObj.commandData.score.leaderboardPlayerInfo.id !== bannedIDs.includes(jsonObj.commandData.score.leaderboardPlayerInfo.id) {
					...
					if (jsonObj.commandData.score.rank <= 58 || jsonObj.commandData.score.weight >= 0.4903952634930577) /*Check if user is top100 on map */ {
						if (jsonObj.commandData.score.modifiers == "") /*Check if user is using modifier */ {
							if (jsonObj.commandData.leaderboard.ranked == true) /*Check if score is on ranked map */ {
								...
								const findMax = (difficulty, data) => {
								let BmaxScore = [] //Create notes array
								data.forEach(diff => { //For each difficulty
									if (diff.difficulty === difficulty) { 
										BmaxScore = diff.maxScore //Set notes to difficulty notes
									//console.log(notes); //Log notes
									} 
								}) 
									return BmaxScore //Return notes
								}
								var passDiff = findNotes(songDiff, data.versions[0].diffs); //Find notes for song difficulty
								var BMax = findMax(songDiff, data.versions[0].diffs); //Find max score
								var maxScore = (((passDiff - 13) * 920) + 4715); //Calculate max score
								// var acc = (modifiedScore / BMax * 100); //Calculate accuracy
								var acc = (modifiedScore / maxScore * 100); //Calculate accuracy
									...
									//console.log(cr); //Log cr
										...
										if (acc >= 86.9) {
											console.log(""+name+" ("+id+") just set a score on "+beatsaver+" ("+songName+" - "+songAuthorName+" by "+levelAuthorName+")"); //Log everything
